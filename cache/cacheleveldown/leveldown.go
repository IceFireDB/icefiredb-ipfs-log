...
func (l *levelDownCache) Load(directory string, dbAddress string) (ds datastore.Datastore, err error) {
...
func datastoreKey(directory string, dbAddress string) string {
	//dbPath := path.Join(dbAddress.GetRoot().String(), dbAddress.GetPath())
	keyPath := path.Join(directory, dbAddress)

	return keyPath
}

func (l *levelDownCache) Destroy(directory string, dbAddress string) error {
	keyPath := datastoreKey(directory, dbAddress)
	l.muCaches.Lock()
	defer l.muCaches.Unlock()
